...
class Setup(commands.Cog):
    ...
    @commands.hybrid_group(description="Setup commands.", invoke_without_command=True)
    ...
    @setup.command(description="Makes a muted role & setups in the server.")
    @commands.check_any(commands.is_owner(), commands.has_permissions(manage_guild=True))
    async def muterole(self, ctx: commands.Context):

        if "muted" in map(lambda i: i.name, ctx.guild.roles):
            await ctx.send(f"{config.Cross} | This Server Already Contains A muted-role")
            return
            
        await ctx.send(f"Setting up a mute role...")

        role = await ctx.guild.create_role(name="muted", reason=f"By {ctx.author}: Created muted role.")
        ...
        await ctx.send(f"{config.Tick} | Successfully Created & Setup muted role")
    ...
    async def remove1(self, ctx, channel : discord.TextChannel = None):
        ...
        if channel.id in ch_:
            if channel.id == ch_[0]:
                ...
                await ctx.send(f"{config.Tick} | Successfully removed {channel.mention} from media channels.")
            elif channel.id == ch_[1]:
                await cursor.execute("DELETE FROM Media WHERE guild_id = ? AND channel_id = ?", (ctx.guild.id, channel.id))
    ...
    @channel.command(name="config",description="Shows the configuration of media channels.", invoke_without_command=True)
    @commands.check_any(commands.is_owner(), commands.has_permissions(manage_guild=True))
    async def config1(self, ctx):
        ...
        embed = discord.Embed(title=f"{ctx.guild.name} Media Channel Settings <:BlueModeration:1089506519076311052>", color=config.color)
